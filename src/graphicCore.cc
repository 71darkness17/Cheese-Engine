...
void GraphicCore::_addRectangle(glm::vec2 position, float width, float height, glm::vec3 color,
  ...
  figures[index] = {
    .vertexOffset = startNum, .firstIndex = startIndex, .indexCount = 6, .model = glm::mat4(1.0f)};
...
uint32_t GraphicCore::addTriangle(std::array<glm::vec2, 3> positions, glm::vec3 color) {
  ...
  cmd.addTri = {.positions = positions, .color = color, .index = nextFigureHex};
  renderQueue.push(cmd);
...
void GraphicCore::_addTriangle(std::array<glm::vec2, 3> positions, glm::vec3 color,
  ...
  figures[index] = {
    .vertexOffset = startNum, .firstIndex = startIndex, .indexCount = 3, .model = glm::mat4(1.0f)};
...
void GraphicCore::setTransform(uint32_t index, const glm::mat4& transform) {
  ...
  cmd.transform = {.index = index, .model = transform};
  renderQueue.push(cmd);
}
...
void GraphicCore::setCamera(glm::vec2 position, float zoom) {
  RenderCommand cmd;
  cmd.type   = SetCamera;
  cmd.camera = {.position = position, .zoom = zoom};
  renderQueue.push(cmd);
}
...
GraphicCore::TextureDescriptor GraphicCore::addTexture(const std::string &path) {
...
void GraphicCore::setTexture(uint32_t figureIndex, TextureDescriptor textureDescriptor) {
    if(figures.count(figureIndex) == 0)
        return;
    figures[figureIndex].textureHandler = (textureDescriptor.arrayId << 16) | textureDescriptor.layerId;
}
...
void GraphicCore::pollRenderQueue() {
  ...
  while (renderQueue.pop(cmd)) {
    switch (cmd.type) {
      case AddRect:
        _addRectangle(cmd.addRect.position, cmd.addRect.width, cmd.addRect.height,
                      cmd.addRect.color, cmd.addRect.index);
        break;
      case AddTriangle:
        _addTriangle(cmd.addTri.positions, cmd.addTri.color, cmd.addTri.index);
        break;
      case RemoveFig:
...
void GraphicCore::createDescriptorSetLayout() {
  ...
  samplerLayoutBinding.descriptorCount = textureArrays.size();
...
void GraphicCore::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties,
...
void GraphicCore::createTextureImage() {
    for(auto &textureArray : textureArrays){
        ...
        createImage(textureArray.extent.width, textureArray.extent.height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureArray.image, textureArray.imageMemory, textureArray.layersCount);
        
        transitionImageLayout(textureArray.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, textureArray.layersCount);
        copyBufferToImage(stagingBuffer, textureArray.image, static_cast<uint32_t>(textureArray.extent.width), static_cast<uint32_t>(textureArray.extent.height), textureArray.layersCount);
        transitionImageLayout(textureArray.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, textureArray.layersCount);
...
void GraphicCore::createTextureImageView() {
    for(auto &textureArray : textureArrays){
        textureArray.imageView = createImageView(textureArray.image, VK_FORMAT_R8G8B8A8_SRGB, textureArray.layersCount);
    }
}
...
void GraphicCore::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t layerCount) {
...
void GraphicCore::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, uint32_t layerCount) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void GraphicCore::createDescriptorPool() {
  ...
  poolSizes[1].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * textureArrays.size());

  VkDescriptorPoolCreateInfo poolInfo{};
  poolInfo.sType         = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
  poolInfo.poolSizeCount = static_cast<uint32_t>(poolSizes.size());
  poolInfo.pPoolSizes    = poolSizes.data();
  poolInfo.maxSets       = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * textureArrays.size());
...
void GraphicCore::createDescriptorSets() {
  ...
  for (size_t i = 0; i != MAX_FRAMES_IN_FLIGHT; ++i) {
    ...
    std::vector<VkDescriptorImageInfo> imageInfos(textureArrays.size());
...
void GraphicCore::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
  ...
  for (const auto& [_, figure] : figures) {
    ...
    vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pc),
                       &pc);
...
bool GraphicCore::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  return properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && features.geometryShader &&
         indicies.isComplete() && extesionsSupported && swapChainAdequate && features.samplerAnisotropy;
}
